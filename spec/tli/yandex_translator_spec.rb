...
describe YandexTranslator do
  before :each do
    @translator = YandexTranslator.new
  end

  describe '#name' do
    it { expect(YandexTranslator).to respond_to(:name) }
    it { expect(YandexTranslator.name).not_to be_nil   }
  end

  describe '#provide_tts?' do
    it { expect(@translator).to respond_to(:provide_tts?) }
    it { expect(@translator.provide_tts?).to be false }
  end

  describe '#langs' do
    it { expect(@translator).to respond_to(:langs) }

    it { expect(@translator.langs).to include('en') }
    it { expect(@translator.langs).to include('es') }
  end
  ...
  describe '#translate' do
    ...
    it 'should fail when source code is not supported' do
      expect { @translator.translate('text', 'English', 'es') }.to raise_error
    end

    it 'should fail when target code is not supported' do
      expect { @translator.translate('text', 'en', 'Spanish') }.to raise_error
    end
  ...
  describe '#info' do
    it { expect(YandexTranslator).to respond_to(:info) }
    it { expect(YandexTranslator.info).not_to be_nil }
  end
end
